...
const updateOrderItemStatus = async (req, res) => {
  try {
    ...
    if (!validTransitions[orderItem.status]?.includes(status)) {
      return res.status(HttpStatus.BAD_REQUEST).json({ success: false, message: 'Invalid status transition' });
    }
    ...
    res.status(HttpStatus.INTERNAL_SERVER_ERROR).json({ success: false, message: 'Error updating item status' });
  }
};
