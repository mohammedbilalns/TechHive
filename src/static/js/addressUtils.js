...
function showError(fieldName, message, formType) {
    const input = document.querySelector(`#${formType}AddressForm [name="${fieldName}"]`);
...
function resetErrors(formType) {
    ...
    form.querySelectorAll('input, select').forEach((input) => {
        ...
        const errorElement = document.getElementById(getErrorId(input.name, formType));
...
function validateAddressData(rawData, formType) {
    ...
    textFields.forEach(([fieldName, label]) => {
        if (!data[fieldName]) {
            ...
            showError(fieldName, `${label} must be between 3 and 50 characters`, formType);
            hasError = true;
        }
    });
    ...
    if (data.alternatePhone && !/^\d{10}$/.test(data.alternatePhone)) {
        showError('alternatePhone', 'Please enter a valid 10-digit alternate phone number', formType);
        hasError = true;
    }
