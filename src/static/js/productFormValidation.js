...
function validateProductData(productData, options = {}) {
  ...
  if (!productData.name) {
    ...
    errors.name =
      "Product name can only contain letters, numbers, spaces, parentheses () and % symbol";
  }
  ...
  if (
    ...
    productData.description.replace(/\s/g, "").length === 0
  ) {
  ...
  const hasEmptySpecification = productData.specifications.some(
    (specification) =>
      !specification || specification.replace(/\s/g, "").length === 0,
  );
  ...
  if (hasEmptySpecification) {
    ...
    errors.specifications =
      "Specifications must be between 10 and 100 characters";
  }
